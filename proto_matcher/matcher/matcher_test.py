...
class ProtoCompareTest(unittest.TestCase):
    def assertProtoCompareToBe(
        self, result: compare.ProtoComparisonResult, to_be: bool
    ):
    ...
    def test_basic_partial_equality(self):
        assert_that(self._get_test_proto(), partially(equals_proto(_TEST_PROTO)))
    ...
    def test_basic_partial_inequality(self):
        ...
        assert_that(self._get_test_proto(), not_(partially(equals_proto(expected))))
    ...
    def test_within_fraction_or_margin_float(self):
        ...
        assert_that(actual, approximately(equals_proto(expected), float_margin=10.0))
        assert_that(actual, approximately(equals_proto(expected), float_fraction=0.2))
        assert_that(
            actual, not_(approximately(equals_proto(expected), float_fraction=0.01))
        )
        assert_that(
            actual,
            approximately(
                equals_proto(expected), float_fraction=0.1, float_margin=10.0
            ),
        )
    ...
    def test_ignore_field_single(self):
        ...
        assert_that(actual, ignoring_field_paths({("baz",)}, equals_proto(expected)))
    ...
    def test_ignore_field_repeated(self):
        ...
        assert_that(actual, ignoring_field_paths({("bars",)}, equals_proto(expected)))
    ...
    def test_ignore_field_multiple(self):
        ...
        assert_that(
            actual, not_(ignoring_field_paths({("baz",)}, equals_proto(expected)))
        )
        assert_that(
            actual, not_(ignoring_field_paths({("bars",)}, equals_proto(expected)))
        )
        assert_that(
            actual, ignoring_field_paths({("bars",), ("baz",)}, equals_proto(expected))
        )
    ...
    def test_ignore_field_nested(self):
        ...
        assert_that(
            actual, ignoring_field_paths({("bars", "size")}, equals_proto(expected))
        )
    ...
    def test_compare_proto_repeated_fields_ignoring_order(self):
        ...
        assert_that(actual, ignoring_repeated_field_ordering(equals_proto(expected)))
    ...
    def test_ignore_nested_field_with_ignore_repeated_field_order(self):
        ...
        assert_that(actual, not_(equals_proto(expected)))
        assert_that(
            actual, not_(ignoring_repeated_field_ordering(equals_proto(expected)))
        )
        ignored_fields = {("bars", "short_id"), ("bars", "long_id")}
        assert_that(
            actual, not_(ignoring_field_paths(ignored_fields, equals_proto(expected)))
        )
        assert_that(
            actual,
